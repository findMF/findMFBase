...
class DetermineBinWidth{
protected:
  std::vector<double> mass_;
  DetermineBinWidth(){SetUp();}
...
BOOST_FIXTURE_TEST_SUITE(DetermineBinWidthTest, DetermineBinWidth)

BOOST_AUTO_TEST_CASE(testSamplingWidth)
{
  ralab::base::resample::SamplingWith sw;
  double bla = sw(mass_.begin(),mass_.end());
  BOOST_CHECK_SMALL(bla - 0.00014 ,0.00001);
}
